...
def run(df):
    ...
    if "valor_mxn" not in df.columns:
        st.error("No se encontró la columna 'valor_mxn', 'valor usd', 'ventas_usd' ni 'importe'.")
        return
    ...
    tabla_fija = pivot_ventas.pivot(index="año", columns="mes", values="valor_mxn").fillna(0)
    ...
    df_chart = tabla_fija.reset_index().melt(id_vars="año", var_name="mes", value_name="valor_mxn")
