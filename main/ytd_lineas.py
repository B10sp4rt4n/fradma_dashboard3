...
def aplicar_color_fondo_linea(val):
...
def crear_grafico_lineas_acumulado(df, año_actual, año_anterior=None, fecha_corte_actual=None,
                                   usar_año_completo_anterior=False):
    """
    ...
    # Datos año actual
    df_actual = calcular_ytd(df, año_actual, fecha_corte_actual)
    ...
    if año_anterior:
        if usar_año_completo_anterior:
            ...
            fecha_corte_actual = _normalizar_fecha_naive(fecha_corte_actual) or obtener_fecha_corte_efectiva(df, año_actual)
    ...
    def calcular_crecimiento_seguro(row):
        ...
        if anterior == 0:
            if actual == 0:
                ...
                return None  # Nueva línea; no existe base comparable
        else:
...
def crear_grafico_comparativo_anos_completos(df, años_disponibles, fecha_corte_actual=None):
    """
    Crea gráfico de barras comparando el avance YTD equivalente de los años disponibles.

    Args:
        df: DataFrame con datos de ventas
        años_disponibles: Lista de años disponibles en los datos
        fecha_corte_actual: Fecha de corte de referencia para homologar el avance histórico

    Returns:
        Figura de Plotly con comparativo histórico consistente con YTD
    """
    ...
    fecha_referencia = _normalizar_fecha_naive(fecha_corte_actual)

    # Calcular ventas por año usando el mismo corte equivalente para todos
    ventas_por_año = []
    for año in años_a_mostrar:
        if fecha_referencia is not None:
            try:
                fecha_corte_hist = datetime(año, fecha_referencia.month, fecha_referencia.day)
            except ValueError:
                fecha_corte_hist = datetime(año, fecha_referencia.month, 28)
        else:
            fecha_corte_hist = None

        df_año = calcular_ytd(df, año, fecha_corte_hist)
    ...
    fig.add_trace(go.Bar(
        ...
        hovertemplate='<b>Año %{x}</b><br>' +
                     'Ventas YTD equivalentes: $%{y:,.2f}<br>' +
                     '<extra></extra>'
    ))
    ...
    fig.update_layout(
        title={
            'text': '<b>Contexto Histórico YTD al Mismo Corte</b>',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 16}
        },
        xaxis_title='Año',
        yaxis_title='Ventas USD YTD',
        height=340,
        template=None,
        paper_bgcolor='white',
        plot_bgcolor='white',
...
def exportar_excel_ytd(df_ytd, año, comparativo_df=None, fecha_corte=None, modo_comparacion="ytd_equivalente"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        resumen_data = {
            ...
            'Valor': [
                ...
                (_normalizar_fecha_naive(fecha_corte) or _normalizar_fecha_naive(df_ytd['fecha'].max())).strftime('%Y-%m-%d') if len(df_ytd) > 0 else 'N/D',
                'YTD equivalente' if modo_comparacion == 'ytd_equivalente' else 'Año completo anterior',
...
def run(df, habilitar_ia=False, openai_api_key=None):
    ...
    # Calcular YTD usando la última fecha real disponible del dataset
    ...
    if año_anterior:
        ...
        if total_anterior > 0:
            ...
            crecimiento_pct = None  # Nuevo sin base comparable
    ...
    with col2:
        ...
        st.metric(
            label=label_crec,
            value=(f"{crecimiento_pct:+.1f}%" if crecimiento_pct is not None else "Nuevo") if año_anterior else "N/A",
            delta_color="off",
    ...
    tab_resumen, tab_lineas, tab_detalle, tab_exportar = st.tabs([
        "Resumen Ejecutivo", "Por Línea", "Detalle", "Exportar"
    ])
    ...
    with tab_resumen:
        if habilitar_ia and openai_api_key and puede_usar_ia:
            ...
            if st.button("🚀 Generar Análisis con IA", type="primary", use_container_width=True, key="btn_ia_ytd_lineas"):
                with st.spinner("🔄 Generando análisis ejecutivo con GPT-4o-mini..."):
                    try:
                        ...
                        proyeccion_anual_filtrado = (ventas_ytd_actual_filtrado / dias_transcurridos_filtrado) * 365 if dias_transcurridos_filtrado > 0 else 0

                        if año_anterior and modo_comparacion == "año_completo":
                            descripcion_comparacion_ia = f"""
⚠️ IMPORTANTE - CONTEXTO DE COMPARACIÓN:
- YTD Actual = Primeros {dias_transcurridos_filtrado} días del año ACTUAL
...
- YTD Actual = Primeros {dias_transcurridos_filtrado} días del año ACTUAL
- YTD Anterior = Primeros {dias_transcurridos_filtrado} días del año ANTERIOR
...
"""
                        ...
                        analisis = generar_resumen_ejecutivo_ytd(
                            ...
                            etiqueta_dimension="LÍNEA DE NEGOCIO"
                        )
        ...
        fig_lineas = crear_grafico_lineas_acumulado(
            df_filtrado,
            año_actual,
            año_anterior,
        ...
        if len(años_disponibles) >= 2:
            st.info(
                f"Lectura sugerida: primero evalúa la evolución acumulada principal. Este bloque solo agrega contexto histórico al mismo corte del {fecha_corte_actual.strftime('%d/%m')}."
            )
            with st.expander("Ver contexto histórico YTD", expanded=False):
                st.caption("Cada barra compara el avance acumulado de cada año hasta la misma fecha equivalente; no representa cierres anuales completos.")
                fig_años_completos = crear_grafico_comparativo_anos_completos(
                    df_filtrado,
                    años_disponibles,
                    fecha_corte_actual=fecha_corte_actual,
                )
    ...
    with tab_lineas:
        ...
        with col_left:
            if año_anterior:
                ...
                for _, row in comparativo_a_mostrar.iterrows():
                    ...
                    if pd.isna(crecimiento):
                        ...
                        emoji_trend = "📈"
                        delta_color = "normal"
                        crecimiento_label = f"{crecimiento:+.1f}%"
                    elif crecimiento < 0:
                        emoji_trend = "📉"
                        delta_color = "inverse"
                        crecimiento_label = f"{crecimiento:+.1f}%"
                    ...
                    with st.expander(f"{emoji_trend} **{linea}** - ${ventas_actual:,.0f} ({crecimiento_label})", expanded=False):
                        col_a, col_b, col_c = st.columns(3)

                        with col_a:
                            st.metric(label=f"Año {año_actual}", value=f"${ventas_actual:,.0f}")
                        with col_b:
                            st.metric(label=f"Año {año_anterior}", value=f"${ventas_anterior:,.0f}")
                        with col_c:
                            st.metric(
                                label="Crecimiento",
                                value=crecimiento_label,
                                delta=f"${variacion_absoluta:+,.0f}" if not pd.isna(crecimiento) else "Sin base comparable",
        ...
        with col_right:
            ...
            st_tabla = ventas_linea_mostrar.style\
                ...
                .applymap(aplicar_color_fondo_linea, subset=['Línea'])
    ...
    with tab_detalle:
        subtab1, subtab2, subtab3 = st.tabs(["📋 Desglose Mensual", "👥 Top Clientes", "📦 Top Productos"])
        ...
        **YTD vs Año Completo**
        ...
        ### 📝 Notas Importantes
                ...
                - **Crecimiento desde $0**: Cuando año anterior = 0 y hay ventas actuales, se reporta como **Nuevo** para evitar porcentajes engañosos
        - **Días del Año Actual vs Histórico**: 
                    - Año actual (ej: 2026): Días desde 01/Ene hasta la última fecha real disponible en el dataset (o hoy, si coincide)
    ...
    with tab_exportar:
        ...
        if puede_exportar:
            ...
            with col_exp1:
                ...
                excel_buffer = exportar_excel_ytd(
                    df_ytd_actual,
                    año_actual,
                    comparativo_df_export,
                ...
                st.caption("Incluye: resumen ejecutivo, desglose mensual, comparativo y top productos/clientes")
    ...
    st.caption(f"📅 Reporte generado: {now_mx().strftime('%Y-%m-%d %H:%M:%S')} | "
              f"Período analizado: 01/01/{año_actual} - {fecha_corte_actual.strftime('%d/%m/%Y')} | "
              f"Modo: {'YTD equivalente' if modo_comparacion == 'ytd_equivalente' else 'Año completo anterior'}")
