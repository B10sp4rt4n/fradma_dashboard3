...
```mermaid
graph TB
    A[FRADMA DASHBOARD] --> B[Módulos Analíticos]
    A --> C[Módulos Operativos]
    A --> D[Módulos Administrativos]
    
    B --> B1[📊 Reporte Ejecutivo ⭐IA]
    B --> B2[📈 Reporte Consolidado ⭐IA]
    B --> B3[📉 KPIs Generales ⭐IA]
    B --> B4[📅 Comparativo Año×Año]
    B --> B5[📊 YTD Línea Negocio ⭐IA]
    B --> B6[🔥 Heatmap Ventas]
    
    C --> C1[💳 KPI Cartera CxC ⭐IA]
    C --> C2[👥 Vendedores + CxC ⭐IA]
    C --> C3[🧮 Herramientas Financieras]
    C --> C4[📦 Ingesta CFDIs]
    
    D --> D1[👤 Gestión Usuarios]
    D --> D2[⚙️ Configuración]
