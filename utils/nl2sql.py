...
SQL: WITH totales AS (SELECT receptor_nombre AS cliente, SUM(total * COALESCE(tipo_cambio, 1)) AS total_mxn FROM cfdi_ventas WHERE empresa_id = '{empresa_id}' AND tipo_comprobante = 'I' AND total > 0 GROUP BY receptor_nombre), con_pct AS (SELECT cliente, total_mxn, ROUND(total_mxn * 100.0 / SUM(total_mxn) OVER (), 2) AS pct_del_total FROM totales), top10 AS (SELECT * FROM con_pct ORDER BY total_mxn DESC LIMIT 10) SELECT cliente, ROUND(total_mxn, 2) AS total_mxn, pct_del_total, CASE WHEN pct_del_total > 50 THEN 'CRÍTICO (>50%)' WHEN pct_del_total > 25 THEN 'ALTO (25-50%)' WHEN pct_del_total > 10 THEN 'MEDIO (10-25%)' ELSE 'BAJO (<10%)' END AS nivel_riesgo FROM top10 UNION ALL SELECT 'Otros' AS cliente, ROUND(SUM(c.total_mxn), 2) AS total_mxn, ROUND(SUM(c.pct_del_total), 2) AS pct_del_total, 'REFERENCIA' AS nivel_riesgo FROM con_pct c WHERE c.cliente NOT IN (SELECT cliente FROM top10) ORDER BY total_mxn DESC;
